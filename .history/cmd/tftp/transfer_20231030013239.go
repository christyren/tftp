...
func (t *transfer) transmit() {
        logger.Printf("start transmiting (write data transfer). writting data to transfer. \n")
        _, err := t.conn.WriteTo(t.sendData, t.addr)
        if err != nil {
                logger.Printf("Error: failed sending a packet. %s", err)
                t.retry = true
        } else {
                logger.Printf("in transmitting. \n")
                t.retry = false
                t.lastOp = time.Now()
        }
...
func wrqHandler(conn net.PacketConn, p tftp.Packet, addr net.Addr) {
        ...
        t.send(&res)
        logger.Printf("t.send(&res) in write request sends ends, though does not mean successful. \n")
