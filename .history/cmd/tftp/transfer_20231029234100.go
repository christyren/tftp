...
func (t *transfer) send(r tftp.Packet) {
        logger.Printf("start sending data. \n")
        t.sendData, _ = r.MarshalBinary()
        logger.Printf("serialize completed for sending data. Now start transmitting. \n")
        t.transmit()
        logger.Printf("transmit completed. \n")
}

// sendD transmits serialized data
func (t *transfer) transmit() {
        logger.Printf("start transmiting. writting data to transfer. \n")
        ...
        if t.retry {
                // If the transmission failed, you can handle retransmission logic here
                // using the customTimeout value.
                // You can sleep for the customTimeout and then retransmit the packet.
                // Example:
                // time.Sleep(customTimeout)
                // t.transmit(customTimeout)
            }
}
...
func wrqHandler(conn net.PacketConn, p tftp.Packet, addr net.Addr) {
        ...
        t.send(&res)
        logger.Printf("t.send(&res) in write request sends successfully. \n")
